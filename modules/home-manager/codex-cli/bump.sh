...
nix shell nixpkgs#nodejs_22 -c npm install --package-lock-only --ignore-scripts --save-exact "@openai/codex@${version}"

pinned_version="$(
  nix shell nixpkgs#nodejs_22 -c node -p \
    "require('./package-lock.json').packages['node_modules/@openai/codex'].version"
)"

nix shell nixpkgs#nodejs_22 -c node -e \
  "const fs = require('fs'); const version = '${pinned_version}'; const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8')); pkg.version = version; pkg.dependencies['@openai/codex'] = version; fs.writeFileSync('package.json', JSON.stringify(pkg, null, 2) + '\n');"
...
build_expr="let flake = builtins.getFlake \"git+file://${repo_root}?dir=hosts/mercury\"; pkgs = flake.inputs.nixpkgs.legacyPackages.x86_64-linux; src = builtins.path { path = \"${package_dir}\"; name = \"codex-cli-src\"; }; in pkgs.callPackage src {}"
...
echo "Pinned @openai/codex ${pinned_version}"
