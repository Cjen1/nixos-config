...
nix shell nixpkgs#nodejs_22 -c npm install --package-lock-only --ignore-scripts --save-exact "t3@${version}"

pinned_version="$(
  nix shell nixpkgs#nodejs_22 -c node -p \
    "require('./package-lock.json').packages['node_modules/t3'].version"
)"

nix shell nixpkgs#nodejs_22 -c node -e \
  "const fs = require('fs'); const version = '${pinned_version}'; const pkg = JSON.parse(fs.readFileSync('package.json', 'utf8')); pkg.version = version; pkg.dependencies['t3'] = version; fs.writeFileSync('package.json', JSON.stringify(pkg, null, 2) + '\n');"
...
build_expr="let flake = builtins.getFlake \"git+file://${repo_root}\"; pkgs = flake.inputs.nixpkgs.legacyPackages.x86_64-linux; src = builtins.path { path = \"${package_dir}\"; name = \"t3-code-src\"; }; in pkgs.callPackage src {}"
