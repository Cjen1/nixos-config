...
Use three confidence levels throughout: **direct**, for an explicit primary-source statement; **supported inference**, for a conclusion that follows from multiple cited facts; and **unknown**, when the record does not answer the question. Preserve contradictions instead of averaging them away.
...
Before spawning investigators, inspect the tools available in the current Copilot session. Use `gh` and git for source control, and map available MCP tools and Copilot Spaces to the evidence categories below.
...
- `model`: `gpt-5.6-sol`
- `mode`: `background`
- Tell it not to edit files or spawn subagents.

Each investigator gets:
1. Its evidence category and the matching available tool.
2. The code anchor from Step 2: file paths, symbols, commit hashes, PR numbers, and ticket IDs.
3. The user's original question.
4. A requirement to return exact citations, null results, contradictions, and confidence.
5. For defensive code, a request to search incident and postmortem evidence.
...
Spawn one synchronous synthesizer subagent:
...
3. The user's original question
4. The direct, supported-inference, and unknown confidence framework above
