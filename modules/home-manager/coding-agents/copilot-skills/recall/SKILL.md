...
Copilot session history is available through `session_store_sql`. Query it by repository, workspace, topic, and a bounded time window. Keep raw session content out of the main context when a focused subagent can summarize it.

1. Classify the request. Recall loads working context across recent chats before you act. A human-readable summary of one known session is a different task. If the user already gave you a full state capsule with paths, branch, and the change, use it and skip the mining.
...
3. Search your chat history with `session_store_sql`. Start with the last 7 days and the current repository. Find candidate session IDs before reading turns, then read only relevant sessions and regions. Skip the current session and obvious noise. For a broad result set, fan out bounded session-ID batches to parallel `explore` agents. Each returns the same schema, one block per session: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts such as PRs, tickets, and branches. Cite the session ID. For one or two sessions, search directly.
